...
Le modèle simule la propagation d'une maladie dans une population en fonction de divers processus biologiques, tels que la transmission de la maladie,  les taux de mortalité liés à la maladie ou naturels, le nombre de naissances, les temps d'immunisations et les temps de latence de la maladie.
...
L'utilisation des polygones pour déterminer les effets des paramètres dépend du contexe (Morris, 1991).
...
# Sortie 1
plot(0.1, 0.1, type="n", ylim=c(0, 0.006), xlim=c(0, 0.06), xlab="mu.star", ylab="sigma")
...
# Sortie 2
plot(0.1, 0.1, type="n", ylim=c(0, 0.07), xlim=c(0, 0.30), xlab="mu.star", ylab="sigma")
polygon(x = c(0,0,100), y = c(0,100,0), col = "skyblue", border = NA)
polygon(x = c(0,0,0.1), y = c(0,100,0), col = "yellow", border = NA)
...
En comparant les deux approches, on constate que l'approche OAT se concentre sur des résultats avec des objectif qualitative spécifiques à chaque parametres, tandis que la méthode Morris offre une perspective plus globale sur les effets des paramètres (seule et/ou avec interaction). Néanmoins, on retrouve des conclusions similaires concernant les paramètres les plus influents, qui sont principalement "loss", "trans", "portée" et "K".
...
Proposition de modification du modèle : Lorsque la proportion de décès dus à la maladie par rapport à l'effectif total de la population devient trop élevée, tous les individus testés positifs sont dirigés vers le compartiment de la "quarantaine". Ce mécanisme s'enclenche uniquement lorsque le nombre de décès attribuables à la maladie parmi les individus infectés dépasse un certain "seuil" de la population totale, défini comme suit :
$N_{i,t}\times madd < seuil \times N_t$.  Dans ce compartiment quarantaine, les individus présentent à la fois des taux de mortalité naturelle $(m_{1:3})$ identiques à ceux des autres états de santé, ainsi que des taux de mortalité liés à la maladie (madd) similaires à ceux des individus infectieux. De plus, les individus en quarantaine peuvent développer une immunité au bout d'une certaine période (rec).
...
- Dans le cas où le nombre de morts liés à la maladie est inférieur au seuil : 
...
## Figures   ***On pourrait peut être enlever cette partie***
...
# Sortie 1
plot(0.1, 0.1, type="n", ylim=c(0, max(sigma$ycol1)+0.1), xlim=c(0, max(mu.star$ycol1)+0.1), xlab="mu.star", ylab="sigma")
polygon(x = c(0,0,100), y = c(0,100,0), col = "skyblue", border = NA)
polygon(x = c(0,0,0.1), y = c(0,100,0), col = "yellow", border = NA)
polygon(x = c(0.1,100,0.1), y = c(0,0.1,0.1), col = "orange", border = NA)
points(mu.star$ycol1, sigma$ycol1)
...
# Sortie 2
plot(0.1, 0.1, type="n", ylim=c(0, max(sigma$ycol2)+0.01), xlim=c(0, max(mu.star$ycol2)+0.01), xlab="mu.star", ylab="sigma")
polygon(x = c(0,0,100), y = c(0,100,0), col = "skyblue", border = NA)
polygon(x = c(0,0,0.1), y = c(0,100,0), col = "yellow", border = NA)
polygon(x = c(0.1,100,0.1), y = c(0,0.01,0.01), col = "orange", border = NA)
points(mu.star$ycol2, sigma$ycol2)
...
# Sortie 3
plot(0.1, 0.1, type="n", ylim=c(0, max(sigma$ycol3)+0.2), xlim=c(0, max(mu.star$ycol3)+1), xlab="mu.star", ylab="sigma")
polygon(x = c(0,0,100), y = c(0,100,0), col = "skyblue", border = NA)
polygon(x = c(0,0,0.1), y = c(0,100,0), col = "yellow", border = NA)
polygon(x = c(0.1,900,0.1), y = c(0,0,0.1), col = "orange", border = NA)
points(mu.star$ycol3, sigma$ycol3)
...
# Sortie 4
plot(0.1, 0.1, type="n", ylim=c(0, max(sigma$ycol4)+2), xlim=c(0, max(mu.star$ycol4)+5), xlab="mu.star", ylab="sigma")
polygon(x = c(0,0,200), y = c(0,100,0), col = "skyblue", border = NA)
polygon(x = c(0,0,0.1), y = c(0,5000,0), col = "yellow", border = NA)
polygon(x = c(0.1,5000,0.1), y = c(0,0,0.1), col = "orange", border = NA)
points(mu.star$ycol4, sigma$ycol4)

...
sorties[[1]] %>%
  kable(format = "html", booktab = FALSE, caption = "Ma table")
...
increment = c(10, 0.05, 0.0001, 0.00001, 
...
Elasticite = list()
matrice = matrix(numeric(0), nrow = 11, ncol = 4)

for (i in 1:15){
  matrice = matrix(numeric(0), nrow = 10, ncol = 4)
  for (j in 1:4){
    elas = sorties_oat[[i]][2:11,j] / parametre[[i]][2:11,i] * ((sorties_oat[[i]][2:11,j]-sorties_oat[[i]][1:10,j]) / (parametre[[i]][2:11,i] - parametre[[i]][1:10,i]))
    matrice[,j] = elas
  }
  Elasticite[[i]] = matrice
}

# compilation des tableaux

for (i in 1:15){
sortie_1[,i] <- Elasticite[[i]][,2]
}

colnames(sortie_1) = c("k", "sr", "m1", "m2", "m3", "f2", "f3", "portee", "t1", "t2", "trans", "lat", "rec", "loss","madd")

# violin plots

sortie_1 <- sortie_1 %>%
  pivot_longer(c("k", "sr", "m1", "m2", "m3", "f2", "f3", "portee", "t1", "t2", "trans", "lat", "rec", "loss","madd"))%>%
  rename(parametre = name, Elasticite = value)

sortie_1 %>%
  ggplot(aes( y=Elasticite,  x= parametre)) +
    geom_boxplot()+
  geom_jitter()

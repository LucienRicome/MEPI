...
Les résultats des analyses de sansibilité des trois méthodes (oat, morris et fast) sont globalement similaire. 
...
### Equations différencielles associées

Le modèle est définit par les systèmes d'équations d'ifférentielles suivants: 
...
B_{Q,t+1} = A_{Q,t}\times t_2 + B{Q,t}\times(1-m_1-t_2-madd - rec) \\
...
C_{Q,t+1} = B_{Q,t}\times t_2 + C{Q,t}\times(1-m_1-t_2-madd - rec) \\
...
2. Dans le cas où les individus infectieux testés positifs sont mis en quanrantaine 
...
```
Les quatre sorties issues du scénario présentent des valeurs bien plus faible qu'en absence de scénario. La quarantaine conditionnelle au nombre de mort à l'instant t a donc bien un effet négatif sur la prévalence, la dynamique de transmission et l'état final de la maladie. 
...
#### Figures OAT
Exploration des figures de l'élasticité (sansibilité rélative) des 17 paramêtres.
```{r}
# Représentations graphiques de l'élasticité pour chaque paramètre
...
Les paramètres "eff" et "seuil" n'ont significativement pas influencés les 4 sorties du modèle avec le scénario.
...
Les distributions des échantillonnages des paramètres par la méthode FAST avec 1000 simulations par paramètre ne montrent pas une lois normale. Les indices d'effet principale et d'effet interaction ne sont donc plus interprétables puisque basés sur l'analyse de la variance (VCE). 
...
En comparaison avec le modèle initial, nous avons constaté que les résultats issus de l'analyse de sansibilité OAT  et morris n'ont pas changé avec le modèle avec le scénario. Cependant, avec la méthode FAST, nous avons trouvé que le parametre seuil à une trés grande influence sur les sorties du modèles. 
